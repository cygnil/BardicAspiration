...
The pipeline supports a **plugin hook system** that allows you to run custom scripts at various stages of the processing pipeline. This enables seamless integration of additional functionality (like AI art generation) without modifying the core codebase.

### How Plugins Work

Plugins are defined in the `plugins.json` file in the root directory. It contains arrays of shell commands associated with pipeline lifecycle hooks. Each hook fires at a specific stage, and commands execute sequentially. `run_pipeline.py` will automatically substitute variables into your commands:
...
| Hook | Fires After |
|------|------------|
| `post_transcribe` | Transcription completes |
| `post_extract_samples` | Sample extraction finishes |
| `post_diarize` | Speaker diarization completes |
| `post_annotate` | Annotation processing finishes |
| `post_visualize` | Visualization generation completes |
| `post_summarize` | Session summary generation finishes |
| `post_update_wiki` | Wiki updates complete |
| `post_relink_wiki` | Wiki relinking finishes |
| `post_recap` | Recap generation completes |
| `pipeline_complete` | Entire pipeline finishes successfully |

### Example: Art Generation Plugin

To run an art generation plugin immediately after summary creation, simply define it in your `plugins.json`:

```json
{
  "post_summarize": [
    "python plugins/generate_art.py --campaign {campaign} --session {session}"
  ]
}
```

The orchestrator will replace the wildcard arguments and run it as an external sub-process. Because it uses standard shell execution under the hood, your plugins can be written in Node.js, bash, Go, or any language you prefer.
